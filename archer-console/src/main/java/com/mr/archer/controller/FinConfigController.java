...
import com.mr.archer.service.FinConfigService;
...
@Slf4j
@RestController
@RequestMapping(SystemConstant.APP_CONTEXT + "/finconfig")
public class FinConfigController extends BaseController {

  @Autowired
  private FinConfigService finConfigService;

  @PermInfo("查询财报配置信息列表信息")
  @PostMapping("/list")
  public Json query(@RequestBody String body) {
    String oper = "query FinConfig";
    log.info("{}, body: {}", oper, body);
    JSONObject json = JSON.parseObject(body);

    Wrapper<FinConfig> queryParams = new EntityWrapper<>();
    ...
    Page<FinConfig> page = finConfigService.selectPage(PageUtils.getPageParam(json), queryParams);
  ...
  public Json createData(@RequestBody String body) {
    String oper = "create FinConfig";
    log.info("{}, body: {}", oper, body);
    FinConfig newData = JSON.parseObject(body, FinConfig.class);
    ...
    finConfigService.insert(newData);
  ...
  public Json queryData(@RequestBody String body) {
    ...
    FinConfig data = finConfigService.selectById(sId);
  ...
  public Json updateData(@RequestBody String body) {
    String oper = "update FinConfig";
    log.info("{}, body: {}", oper, body);
    FinConfig data = JSON.parseObject(body, FinConfig.class);
    SysUser curUser = getCurUser();
    String sCurUserId = String.valueOf(curUser.getId());
    String sCurUserOrg = curUser.getOrgid();
    String sCurTime = DateUtils.getNowTime();
    ...
    finConfigService.updateById(data);
  ...
  public Json deleteData(@PathVariable String uid) {
    String oper = "delete FinConfig";
    finConfigService.deleteById(uid);
